...
BEGIN
  ...
  --- perform action
  IF NOT EXISTS (SELECT FROM ctfnote.work_on_task as wot WHERE profile_id = user_id AND wot.task_id = t_id) THEN
...
BEGIN
  ...
  --- perform action
  IF EXISTS (SELECT FROM ctfnote.work_on_task as wot WHERE profile_id = user_id AND wot.task_id = t_id) THEN
