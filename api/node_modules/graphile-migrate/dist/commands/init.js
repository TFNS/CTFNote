...
async function init(options = {}) {
    if (await _common_1.exists(_common_1.GMRC_PATH)) {
        throw new Error(`.gmrc file already exists at ${_common_1.GMRC_PATH}`);
    }
    if (await _common_1.exists(_common_1.GMRCJS_PATH)) {
        throw new Error(`.gmrc.js file already exists at ${_common_1.GMRCJS_PATH}`);
    }
...
`;
    await fs_1.promises.writeFile(_common_1.GMRC_PATH, `\
/*
...
`);
    ...
    console.log(`Template .gmrc file written to '${_common_1.GMRC_PATH}'; please read and edit it to suit your needs.`);
    const settings = await _common_1.getSettings();
