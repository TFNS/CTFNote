...
var PgOrderComputedColumnsPlugin = function PgOrderComputedColumnsPlugin(builder) {
  builder.hook("GraphQLEnumType:values", (values, build, context) => {
    ...
    const compatibleComputedColumns = introspectionResultsByKind.procedure.reduce((memo, proc) => {
      ...
      if (!proc.tags.sortable) return memo; // Must not be omitted

      if (omit(proc, "execute")) return memo; // Must be a computed column
      ...
      const {
        pseudoColumnName
      } = computedColumnDetails; // Must have only one required argument
      ...
      if (isVoid) return memo; // Looks good
    ...
    return extend(values, compatibleComputedColumns.reduce((memo, {
      proc,
      pseudoColumnName
    }) => {
...
exports.default = PgOrderComputedColumnsPlugin;
