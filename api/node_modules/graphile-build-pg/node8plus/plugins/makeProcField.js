...
function makeProcField(fieldName, proc, build, {
  ...
  return fieldWithHooks(fieldName, ({
    ...
    return {
      description: proc.description ? proc.description : isMutation ? null : isTableLike && proc.returnsSet ? build.wrapDescription(`Reads and enables pagination through a set of \`${TableType.name}\`.`, "field") : null,
      ...
      resolve: computed ? (data, _args, resolveContext, resolveInfo) => {
        ...
        if (isMutation) {
          ...
          const query = makeQuery(parsedResolveInfoFragment, resolveInfo.returnType, sqlMutationQuery, functionAlias, null, resolveContext, resolveInfo);
          const {
            text,
            values
          } = sql.compile(query);
