...
export async function getTaskFromId(taskId: bigint): Promise<string[]> {
    ...
    try {
        ...
        const query = "SELECT title,category,ctf_id FROM ctfnote.task WHERE id = $1";
        const values = [taskId];
        const queryResult = await pgClient.query(query, values);
        // Extract the "name" field from each row
...
export async function createTask(title: string, description: string, category: string, flag: string, padUrl: string, ctfId: number): Promise<void> {
    const pgClient = await connectToDatabase();
