...
import {CategoryChannel, ChannelType, GuildBasedChannel} from "discord.js";
import {getCTFNameFromId, getNameFromUserId, getTaskFromId} from "../discord/database/ctfs";
        ...
        const handleDiscordMutationLog = async (
            ...
            if (fieldContext.scope.fieldName === "deleteTask") {
                ...
                const channel = guild?.channels.cache.find(
                    (channel) =>
                        channel.type === ChannelType.GuildText && channel.name === `${taskTitle}-${taskCategory}`
                ) as CategoryChannel | undefined;
            ...
            if (fieldContext.scope.fieldName === "updateTask" && args.input.patch.flag !== null && args.input.id !== null) {

                if (args.input.patch.flag !== "") {
                    const task = await getTaskFromId(args.input.id);
                    const taskTitle = task[0].toLowerCase();
                    const taskCategory = task[1].toLowerCase();
                    sendMessageFromTaskId(args.input.id, `${taskTitle} is solved!`).then((channel) => {
                        if (channel !== null) {
            ...
            if (fieldContext.scope.fieldName === "startWorkingOn") {
                ...
                usernamePromise.then((username) => {
                    sendMessageFromTaskId(taskId, `${username} is working working on this task!`);
                });
            ...
            if (fieldContext.scope.fieldName === "stopWorkingOn") {
                ...
                usernamePromise.then((username) => {
                    sendMessageFromTaskId(taskId, `${username} stopped working on this task!`);
                })
...
async function sendMessageFromTaskId(id: bigint, message: string): Promise<GuildBasedChannel | null> {
